...
trait SpecScalatest[P[_]] extends Spec[P]{ self : FunSpec with Matchers =>
  ...
  //   it("should work for working programs"){
  //     trueProgram.shouldBe(true) should runWithoutErrors(from = 0)
  //     falseProgram.shouldBe(false) should runWithoutErrors(from = 0)
  //     workingProgram.shouldBe(()) should runWithoutErrors(from = 0)
  //   }
